...
P_mt_c <- function(t,m,lamb,delta){
  W <- c()
  Prob <- c()
  for (i in 1:m) {
    iter = 25000
    for (j in 1:iter){
      W[j] <-  t < T_m_c(i, lamb, delta)
    }
    Prob[i] <- sum(W)/iter
  }
  return(Prob)
...
P_mt_c <- P_mt_c(t_cross_section,m,lamb,delta)


plot(P_mt,main="Exponential Hacking and Exponential Detecting Times", xlab = "m", ylab=expression("P"[m]*"(t)"),col="#009999")


points(P_mt_c,col="red")
legend(1, 0.9, legend=c("continuous monitoring", "periodic monitoring"),
       col=c("red", "#009999"), lty="dotted", cex=0.8)
